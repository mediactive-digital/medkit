{!! Form::open(['route' => ['back.mailTemplates.destroy', $id], 'method' => 'delete']) !!}
<div class='btn-group'>
    <?php 
    $mailTemplates = \App\Models\MailTemplate::where('id', '=', $id)->first();
    ?> 
    <a href="{{ route('back.mailTemplates.test', $id) }}" class='btn' data-toggle="tooltip" data-placement="right" title="{{ _i('Tester le mailTemplate') }}">
		<i class="material-icons text-Secondary">mail</i>
	</a>
	  
    @if(auth()->user()->can('view', $mailTemplates))  
        <a href="{{ route('back.mailTemplates.show', $id) }}" class='btn'>
           <i class="material-icons text-success">remove_red_eye</i>
        </a>
    @endif
	
    @if(auth()->user()->can('update', $mailTemplates))   
        <a href="{{ route('back.mailTemplates.edit', $id) }}" class='btn'>
